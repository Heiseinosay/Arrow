...
class BirdsEyeView : AppCompatActivity(), FragmentToActivitySearch  {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnTest.setOnClickListener {
            ...
            val uri = Uri.parse("android.resource://" + packageName + "/" + R.drawable.polygon1)
    ...
    private fun explorationView(){
        var coordinateSize = Coordinates.gastamToLualhati.size
        val coordinateToUse = Coordinates.gastamToLualhati.toMutableList()
        if (currFloor >= 8){
            coordinateSize += Coordinates.eightFloor.size - 1
            coordinateToUse += Coordinates.eightFloor
        } else{
            coordinateSize += Coordinates.firstFloor.size - 1
            coordinateToUse += Coordinates.firstFloor
        }
        ...
        for (i in 0 until coordinateSize){
            ...
            polylineID.addProperty("imageURI", "$i.png")
        ...
        val clickListener = OnPolylineAnnotationClickListener { polyline ->
            ...
            downloadURL("8th-floor", imageURI)
    ...
    // PANORAMA
    private fun downloadURL(floor:String, polyname: String?){
        val storage = FirebaseStorage.getInstance("gs://arrow-51e15.appspot.com")
        val storageRef = storage.getReference("$floor/8th-$polyname")

        storageRef.downloadUrl.addOnSuccessListener { uri ->

            Log.d("downlaodURL", "Downloaded image URI: $uri")
            Panorama.getInstance()
                .loadImageInfo(this, uri, PanoramaInterface.IMAGE_TYPE_RING)
                .setResultCallback(ResultCallbackImpl())
